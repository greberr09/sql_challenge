...
The final project documents consist of three sql files, two ERD diagrams, an output message log of combined returned messages from the different data analysis queries, and eight output .csv files.  There is one sql file, "employees_schema.sql," to create the database tables and constraints; one sql file, "employees_data_load.sql," to load the .csv files into the database, and one sql file, "employees_data_display," to run the eight analysis queries.  

The online application QuickDBD (opendatabasediagram.com) was used to create one of the ERDs, as suggested as an option in the challenge instructions.  An initial SQL for all six tables was created manually using CODE, and uploaded into QuickDBD, and later finalized manually using Code.   The Postgress SQL produced by the QuickDBD app as an "export" had many issues, including with putting tables and field names in quotation marks that are then included in the field or table defintion, datatypes not known to postgress, and adding all constraints separately by updating the tables after the fact.  So, after the SQL Was output from this tool for examination, it was not used for this project.  
...
The advantage of QuickDBD's less-detailed diagram is that the table names are easy to see and stand out, and look more like something that non-technical executives viewing slides might prefer.  The advantage of the PgAdmin4 ERD program is the greater detail even in the "lesser" detail mode, the greater use of color and images, the easier-to-see relationship indicators, and the use of the proper data types, because the ERD has moved past the conceptual level.  Both diagrams are included in the submission.  Each probably would work better for different audiences.

The project assumes that the "employees_db" is already created and the user running the sql files has the appropriate access.  The database used for this project was created using the pgAdmin4 GUI.  Although a sql to create the database is available in pgAdmin4 after the database is created using the GUI, and is not too complex, a more complex script would be needed to search out any connected PIDs, and to terminate those processes, if the script were to drop and re-create the "employees_db" itself.   This is outside the scope of the requirements, which did not ask for any tyow id script to create the database.  It also could be a problem in a multi-user environment to just terminate running processes, so the database was created manually using the PgAdmin4 GUI.  If connections to the database are open and the user wants to drop and add it, PIDs to delete can be found by running Python scripts, or pgAdmin4 has a transactions GUI that shows connected PIDs, which can then be manually terminated.
