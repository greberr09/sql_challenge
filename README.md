...
The project assumes that the "employees_db" is already created and the user running the sql files has the appropriate access.  The database used for this project was created using the pgAdmin4 GUI.  Although a sql to create the database and set permissios is available in pgAdmin4 after the database is created using the GUI, and is not too complex, a more complex script would be needed to search out any connected PIDs, and to terminate those processes, if the script were to drop and re-create the "employees_db" itself.   This is outside the scope of the requirements, which did not ask for any type of script to create the database.  It also could be a problem in a multi-user environment to just terminate running processes, so the database was created manually using the PgAdmin4 GUI.  If connections to the database are open and the user wants to drop and add it, PIDs to delete can be found by running Python scripts, or pgAdmin4 has a dashboard GUI that shows connected PIDs, which can then be manually terminated.
